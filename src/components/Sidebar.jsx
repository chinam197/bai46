...
const { setCartQuantity, setShowCart, setCartQuantityIncrement } =
...
const Sidebar = () => {
  ...
  useLayoutEffect(() => {
    const numberQuantity = JSON.parse(localStorage.getItem("my_cart"));
    if (numberQuantity && Array.isArray(numberQuantity)) {
      dispatch(setCartQuantity(0));
      numberQuantity.forEach(({ quantity }) => {
        dispatch(setCartQuantityIncrement(quantity));
      });
    }
  }, []);
  const handleShowCart = (path) => {
    navigate(path);
  };
      ...
      <div id="icon">
        <img
          ...
          onClick={() => {
            ...
            handleShowCart("/product/1");
            toast.success("Bạn đã trở lại shop!");
          }}
        />
      </div>
      <svg
